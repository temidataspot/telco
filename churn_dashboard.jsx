...
const compareData = Object.entries(MODELS).map(([key, m]) => ({
  name: m.short,
  ...
  "F1 Score":+(m.metrics.f1        * 100).toFixed(1),
  "ROC-AUC": +(m.metrics.roc       * 100).toFixed(1),
  color: m.color.main,
}));
    ...
    <div style={{ minHeight: "100vh", background: "#070b14", fontFamily: "'Segoe UI', sans-serif", color: "#e2e8f0", position: "relative", overflow: "hidden" }}>
      <style>{`
        ...
        * { box-sizing:border-box; }
        ::-webkit-scrollbar { width:6px } ::-webkit-scrollbar-track { background:#0f141e } ::-webkit-scrollbar-thumb { background:#1e293b; border-radius:4px }
      `}</style>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1300, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ opacity: loaded ? 1 : 0, transform: loaded ? "none" : "translateY(-20px)", transition: "all 0.7s ease", marginBottom: 40, textAlign: "center" }}>
          ...
          <h1 style={{ margin: "0 0 10px", fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 900, fontFamily: "'Georgia', serif", background: `linear-gradient(135deg, #e2e8f0 0%, ${activeColor.main} 60%, #94a3b8 100%)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", transition: "all 0.6s ease" }}>
            Customer Churn Prediction
          </h1>
          <p style={{ margin: "0 0 8px", color: "#64748b", fontSize: 15 }}>Telco Industry · Logistic Regression → SMOTE → Random Forest → XGBoost</p>
        ...
        <div style={{ marginTop: 32, textAlign: "center", color: "#334155", fontSize: 12, fontFamily: "'Courier New', monospace" }}>
          Developed with ♥ by <span style={{ color: activeColor.main, transition: "color 0.6s" }}>Temi Priscilla Jokotola</span> &nbsp;·&nbsp; Telco Churn Prediction Project &nbsp;·&nbsp; Built with Python · Scikit-Learn · XGBoost · SHAP
        </div>
      </div>
    </div>
  );
